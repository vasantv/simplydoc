  ...
  function initialize() {
    ...
    latlng = new google.maps.LatLng(12.9759549, 77.6493073);
    var myOptions = {
      zoom: 15,
      center: latlng,
  ...
  function appendResult(id, provName, provAddress, provPhone, provDistance)
  {
		...
		$("#textResults").append('<div itemscope itemtype="http://data-vocabulary.org/Organization" id="result'+id+'" >\
				  <span>'+id+'.</span>\
					<span itemprop="name"><strong>'+provName+'</strong></span>\
				    <span itemprop="distance" class="greyText"> ('+provDistance+' kms)</span><br/>\
				  <address itemprop="address" itemscope itemtype="http://data-vocabulary.org/Address">\
					'+provAddress+'<br/>\
					<abbr title="Phone" itemprop="tel">Ph:</abbr>'+ provPhone +'<br/>\
				  </address>\
			</div>');			  
  }
  ...
  function displayResult(resLatLng, pageNum) { 
		...
		var offPage = (pageNum-1)*10;	

		//post to PHP file to get information and display in HTML document below
		$.post("getNearBy.php", { lat: resLatLng.lat(), long: resLatLng.lng(), radius1: rad1, radius2: rad2, num: 10, offset: offPage}, 
			function(data){				
				if(data == 0) { 				
				...
				var resultCount=(pageNum-1)*10; //starting each page counter appropriately
				...
				//Pagination logic							
				if(pageNum == 1 && resultCount == 10) { //first page - only append next
				...
				else if(pageNum > 1 && (resultCount%10==0)){ //if pageNum = 2+ and next pages exist										
					$("#textResults").append('<div id="resultPages">\
							<ul class="pager">\
							  <li >\
  ...
  function codeAddress(pageNum) {
      ...
      if (status == google.maps.GeocoderStatus.OK) {
		...
		//event tracking
	  	_gaq.push(['_trackEvent', 'searchClick', 'Search', 'searchLatLng', results[0].geometry.location]);
  ...
  $(document).ready(function() {
	...
	$("#myLocation").click(function(){
		...
		if(navigator && navigator.geolocation){
			navigator.geolocation.getCurrentPosition(function(position){
				alert(position.coords.latitude+"::A:"+position.coords.longitude);
				...
				//event tracking
	  			_gaq.push(['_trackEvent', 'myLocClick', 'Search', 'searchLatLng', localLatLng]);

			},geoLocError,{timeout:5000});
		}
		else
		{
			alert("failure!");
	...
	//if the prev page of results is requested
	$("#res1").click(function(){
		rad1 = 0; rad2 = 1;
	...
	//if the next page of results is requested
	$("#res2").click(function(){
		rad1 = 1; rad2 = 5;
		if (localLocation == 0) //address has been provided
		{
